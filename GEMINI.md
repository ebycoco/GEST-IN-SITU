...
├── electron.vite.config.ts       # Config build Electron (aliases, plugins, outDir vers dist/)
├── forge.config.cjs              # Config Electron Forge (packaging)
├── package.json                  # Dépendances et scripts (main pointant sur dist/main/index.js)
...
│
├── dist/                         # ← DOSSIER DE COMPILATION (electron-vite)
│   ├── main/                     # Fichiers compilés Main & Workers
│   ├── preload/                  # Scripts Preload
│   └── renderer/                 # Build UI React
│
├── out/                          # ← DOSSIER DE PACKAGING (electron-forge & Inno Setup)
│   ├── GEST-IN-SITU-win32-x64/   # Version portable packagée (dossier plat)
│   └── make/                     # Distributions compilées (ZIP, Installateurs)
│
├── src/
...
│           │   ├── VerificationSearchPage.tsx # Recherche et vérification en lecture seule/opérateur
│           │   ├── SaisiePage.tsx         # Saisie manuelle de cartes
...
Les fonctions de nettoyage `cleanBirthDate` et `normalizeContact` sont injectées de manière identique et synchronisée dans `src/main/sync/downstream.ts` et `src/main/workers/import-worker.js`.
...
L'instruction de suppression globale dans `queries.ts` désactive temporairement les clés étrangères (`foreign_keys = OFF`). Pour éviter le blocage synchrone du thread principal, l'instruction lourde `db.prepare("VACUUM").run()` est déportée de manière asynchrone dans un `setTimeout(..., 500)`.
...
Un **debounce de 3 minutes** est appliqué lors des changements d'état réseau pour filtrer et ignorer les micro-coupures de courant ou de signal sur le terrain.

### 6.2 Capture Dual-Track (Upstream & Downstream)
*   **Upstream (`t_sync_queue`)** : Chaque action locale (création, modification, distribution) est capturée par une transaction SQLite qui ajoute un élément ordonné dans `t_sync_queue`. Le moteur d'Upstream rejoue les requêtes sur le cloud par lots transactionnels de 50.
...
*   **Worker Thread et Sync de masse** : Lors de l'import massif de 200k+ lignes, le Worker Thread ne peuple pas la file de synchronisation, mais marque simplement toutes les cartes importées avec `is_dirty = 1`. L'envoi est alors délégué au **Bulk Uploader Initial**.
...
*   **Enregistrement Persistant** : L'application insère un log d'audit système spécial de type `'SYNC_UPDATE'` dans la table locale `t_logs` avec l'état non lu (`valeur_apres = '{"read": false}'`) et marqué `is_dirty = 1`.
...
*   **Badge d'Alerte Différé** : Si l'application était fermée ou le consultant déconnecté, la `TopBar.tsx` interroge le nombre de logs non lus de ce type. S'il y en a, un badge thématique jaune Plein Soleil s'affiche sur la cloche.

### 6.6 Flux des anomalies physiques (Cartes Absentes)
1. **Signalement Consultant (VerificationSearchPage.tsx) :**
   - Sécurité Doublon : Le bouton "Signaler" est désactivé si `carte.statut_physique === 'ABSENT'`, affichant "⏳ En cours de traitement".
2. **Isolation Strict par Sous-Site / Box :**
   - Les notifications de résolution ou de perte sont strictement cloisonnées via `site_id`.
3. **Traitement Admin (AdminQueuePage.tsx & queries.ts) :**
   - Saisie forcée en MAJUSCULES en temps réel pour le champ "Nouveau Rangement" via `.toUpperCase()`.
   - Deux issues possibles : 
     * Rangement trouvé -> `resoudreAbsence(id_carte, nouveau_rangement)`.
     * Rangement introuvable -> `declarerPerdue(id_carte)`.
   - Clôture Atomique : La soumission de l'admin passe automatiquement l'ancien log 'CARTE_ABSENTE_SIGNALEE' à `read: true`.
4. **Notifications Temps Réel & Deep Linking :**
   - Synchro : Le thread Main émet le signal global `sync:updated-data` après chaque action admin.
   - UX : Au clic sur une notification 'CARTE_ABSENTE_RETROUVEE' ou 'CARTE_PERDUE_CONFIRMEE', le consultant ouvre une Modal récapitulative dédiée (Plein Soleil - fort contraste) affichant le Nouveau Rangement ou la procédure de duplicata.
...
*   **SUPER ADMIN** : Accès total, gestion multi-sites (Infrastructures), réinitialisations, dashboard global. Un Switcher dans la Sidebar permet de choisir le site actif.
...
*   **OPERATEUR_QUALITE** : Voit la page d'Assainissement et la page Cartes. Les boutons "Nouvelle Carte" et "Export CSV" lui sont masqués. Il peut modifier ou distribuer des cartes.
...
*   **CONSULTANT** : Recherche uniquement (lecture seule). Tous les boutons d'édition, de modification, de signalement d'absence et de délivrance lui sont masqués.
...
Au chargement, `ImportPage.tsx` appelle `db:getCardCount`. Si le nombre de cartes locales est égal à 0, le bouton "Purger la base" est automatiquement désactivé.
...
| `sync`         | `getStatus`, `force`, `startBulk`, `getUnreadCount`, `markAsRead`, `onDatabaseUpdated` |
...
- Les dates de naissance sont obligatoirement normalisées en format ISO `YYYY-MM-DD`.
- N'altérez jamais les migrations antérieures (v1 à v9). Pour ajouter une modification de structure, incrémentez `SCHEMA_VERSION` à 10 et ajoutez une migration idempotente.
...
- Les modifications manuelles de doublons offline doivent appliquer les fonctions unifiées de normalisation des chaînes de caractères.

---

## 11. [ÉTAT DU PROJET] - Architecture de Build et Distribution

L'application sépare distinctement les fichiers de compilation du code source et les fichiers de packaging :

- **`dist/` (Dossier de Compilation)** : Géré exclusivement par `electron-vite`. Tous les fichiers compilés (Main Process, Preload, et Renderer React) y sont stockés. C'est ce dossier qui est importé et lu au runtime par Electron dans l'archive `app.asar`.
- **`out/` (Dossier de Packaging)** : Contient les distributions portables prêtes à l'exécution (`out/GEST-IN-SITU-win32-x64/`) ainsi que les installateurs ZIP et Setup (`out/make/`).

---

## 12. [PROCÉDURE DE BUILD] - Génération des Exécutables

Pour générer une nouvelle version prête à être installée ou déployée sur Windows, suivez cette séquence exacte :

1. **Compilation et Packaging Electron Forge (ZIP)** :
   ```bash
   npm run make:win
   ```
   *Cette commande compile le code source vers `dist/`, génère le dossier autonome de l'application dans `out/GEST-IN-SITU-win32-x64/` et crée l'archive ZIP dans `out/make/zip/win32/x64/`.*

2. **Création du Setup d'installation Windows (Inno Setup)** :
   - Ouvrez l'application **Inno Setup**.
   - Ouvrez le fichier de script [installer.iss](file:///d:/Espace travail/GEST_IN-SITU_CARTE_ABOBO_V2/installer.iss) disponible à la racine du projet.
   - Cliquez sur **Compile** (ou appuyez sur `Ctrl+F9`).
   - L'exécutable d'installation finale sera généré dans `out/make/installer/GEST-IN-SITU-Setup.exe`.

---

## 13. [POINTS D'ATTENTION] - Limitations d'Electron Packager

> [!IMPORTANT]
> **Ne configurez jamais `electron-vite` pour compiler dans le dossier `out/`.**  
> Par défaut, `electron-packager` (utilisé en interne par Electron Forge) exclut automatiquement tout répertoire nommé `out/` de l'archive `app.asar` car il s'agit de son répertoire de travail et de sortie. Si vos sources compilées y sont placées, l'application se lancera avec une erreur critique signalant que le point d'entrée n'a pas pu être trouvé (`Cannot find module out/main/index.js`).
>
> **La compilation doit impérativement cibler `dist/`**, qui est le répertoire autorisé et correctement packagé à l'intérieur de l'ASAR.

---

## 14. [HISTORIQUE DES CORRECTIFS] - Résolution du Bug de Point d'Entrée (ASAR)

### Problème rencontré (Juillet 2026) :
Lors du premier packaging, le lancement de l'application installée échouait avec une boîte de dialogue d'erreur :
`A JavaScript error occurred in the main process: Uncaught Exception: Cannot find module 'out/main/index.js'`.

### Causes identifiées :
1. L'application utilisait un fichier relais (`index-relais.js`) configuré pour basculer dynamiquement entre `./main/index.js` et `./out/main/index.js` selon la détection de l'archive `.asar`.
2. Le dossier `out/` étant exclus par `electron-packager` lors de la création de l'ASAR, aucun des chemins n'était accessible au runtime dans le package final.

### Correction appliquée :
- Déplacement du dossier de sortie d'electron-vite de `out/` vers `dist/` dans [electron.vite.config.ts](file:///d:/Espace travail/GEST_IN-SITU_CARTE_ABOBO_V2/electron.vite.config.ts).
- Mise à jour du point d'entrée `"main"` dans [package.json](file:///d:/Espace travail/GEST_IN-SITU_CARTE_ABOBO_V2/package.json) directement vers `./dist/main/index.js`, supprimant le besoin d'utiliser un fichier relais.
- Mise à jour des exclusions dans [forge.config.cjs](file:///d:/Espace travail/GEST_IN-SITU_CARTE_ABOBO_V2/forge.config.cjs) et du script d'installation [installer.iss](file:///d:/Espace travail/GEST_IN-SITU_CARTE_ABOBO_V2/installer.iss) pour pointer vers les bons chemins.
- Résultat : Packaging correct et démarrage instantané de l'application sans erreur de bundle.
