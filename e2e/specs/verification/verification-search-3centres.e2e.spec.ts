...
const SEED_3_CENTRES_SCRIPT = join(__dirname, '../../fixtures/seeds/seed-3-centres.ts');
...
// e2e/fixtures/seeds/, même technique de bundling esbuild que les specs
// cloud de ce dépôt — better-sqlite3 est un module natif ABI Electron, ne
// peut pas être exécuté tel quel sous le Node système du test-runner) ──────
async function bundleSeed3CentresScript(): Promise<string> {
  const result = await build({
    entryPoints: [SEED_3_CENTRES_SCRIPT],
    bundle: true,
    platform: 'node',
    format: 'cjs',
    target: 'node20',
  ...
  if (!output) throw new Error('[E2E][seed-3-centres] esbuild n\'a produit aucune sortie.');
  return output;
}

async function runSeed3Centres(userDataDir: string): Promise<Seed3CentresResult> {
  const bundleCode = await bundleSeed3CentresScript();
  const bundleDir = mkdtempSync(join(tmpdir(), 'gest-in-situ-e2e-seed3c-'));
  const bundlePath = join(bundleDir, 'seed-3-centres.bundle.cjs');
  ...
  if (!line) {
    throw new Error(`[E2E] seed-3-centres.ts n'a produit aucun résultat exploitable.\n--- stdout ---\n${stdout}\n--- stderr ---\n${stderr}`);
  }
