...
const EXTRA_SEED_SCRIPT = join(__dirname, '../../fixtures/seeds/extra-seed-qualite.ts');
...
async function bundleExtraSeedQualiteScript(): Promise<string> {
  ...
  if (!output) throw new Error('[E2E][extra-seed-qualite] esbuild n\'a produit aucune sortie.');
  return output;
}

async function runExtraSeed(userDataDir: string, siteId: number, centreId: number): Promise<ExtraSeedResult> {
  const bundleCode = await bundleExtraSeedQualiteScript();
  const bundleDir = mkdtempSync(join(tmpdir(), 'gest-in-situ-e2e-extraseedq-'));
  const bundlePath = join(bundleDir, 'extra-seed-qualite.bundle.cjs');
  ...
  if (!line) {
    throw new Error(`[E2E] extra-seed-qualite.ts n'a produit aucun résultat exploitable.\n--- stdout ---\n${stdout}\n--- stderr ---\n${stderr}`);
  }
