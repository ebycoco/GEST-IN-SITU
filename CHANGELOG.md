...
**Migration `SCHEMA_VERSION` 69 → 70** (additive, aucune perte de données) : `migrateV70` ajoute la colonne `action_at` à `t_cartes` (horodatage d'action métier, distinct du watermark réseau `updated_at`), avec backfill automatique (`action_at = updated_at`) — limite connue et acceptée sur la valeur de départ du backfill pour les cartes déjà envoyées via un circuit d'envoi différé plus ancien, sans impact fonctionnel (voir détail dans le code de la migration). Validé par `agent-12-deploy-validator` (GO officiel) et `npx tsc --noEmit` : 0 erreur.
