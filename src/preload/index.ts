...
const api = {
  ...
  auth: {
    ...
    onSessionExpired: (callback: (payload?: { reason?: 'revoked' | 'disabled' | 'site_suspended' | 'license_expired' }) => void) => {
  ...
  sync: {
    ...
    onCardsReceived: (callback: (data: {
      ...
      updatedCount?: number;
      insertedInMyCentre?: number;
      siteId?: number;
      cards: { noms: string; prenoms: string; rangement: string | null; sync_id: string; isNew?: boolean }[];
    }) => void) => {
